...
CreateCommand.prototype.create_project = function(paths, platform, plugin, callback) {
    ...
    this.tester.emit('log', 'Creating plugin project ' + plugin + ' for platform ' + platform + '.');
    ...
    for(plugin in plugins) {
        for(platform in platforms) {
            if(this.result_obj[platform][plugin].built) {
                //installl required plugins (device and network status)
                this.install_plugin(paths, path.join(paths.projects_dir, platform, plugin), platform, 'cordova-plugin-network-information', callback);
                this.install_plugin(paths, path.join(paths.projects_dir, platform, plugin), platform, 'cordova-plugin-device', callback);
            ...
            if(this.result_obj[platform][plugin].built) {
                if(!fs.existsSync(path.join(paths.plugins_dir, plugin, 'test'))) {
                    ...
                    for(element in www_files) {
                        if(www_files[element] != 'cordova.js' && www_files[element] != 'cordova_plugins.json' && www_files[element] != 'plugins') {
